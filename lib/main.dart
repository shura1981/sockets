...
Future<String> setupInitialRoute() async {
  final initialMessage = await FirebaseMessaging.instance.getInitialMessage();
  if (initialMessage?.data['screen'] == '/home') {
    return '/home';
  }
  return '/';
}
...
Future<void> main() async {
  ...
  final initialRoute = await setupInitialRoute();
  runApp(MyApp(
...
class _MyAppState extends State<MyApp> {
  ...
  void _onMessageOpenApp(RemoteMessage message) {
    ...
    if (message.data.containsKey('screen')) {
      ...
      String title = message.notification?.title ?? 'No data';
      String body = message.notification?.body ?? 'No data';
      final snackBar = SnackBar(content: Text(title));
      scaffoldMessengerKey.currentState!.showSnackBar(snackBar);
      navigatorKey.currentState!.pushNamed(screen, arguments: body);
    }
  }
  ...
  static Future _onMessage(RemoteMessage message) async {
    ...
    if (message.data.containsKey('screen')) {
      ...
      String title = message.notification?.title ?? 'No data';
      String body = message.notification?.body ?? 'No data';
      PushNotificationService.mostarNotificacion(
          title: title, body: body, payload: screen);
    }
  }
      ...
      case 2:
        return Center(
          ...
          onPressed: () => PushNotificationService.mostarNotificacion(
              title: 'Hola', body: 'Mundo', payload: '/home'),
